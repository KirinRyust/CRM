  ...
  fetch("http://144.126.210.74:8080/api/tipo_gestion", requestOptions)
    .then((response) => {
      if(response.status == 200){
        alert("Tipo de gestión creada correctamente")
        location.href="listar.html";
      }
      if(response.status == 400){
        alert("Error al agregar tipo de gestion");
      }
    })
...
function completarEtiqueta(element, index, arr){
  ...
  document.getElementById('lbl_eliminar').innerHTML ="¿Desea eliminar este tipo de gestion?<b> " + nombre_tipo_gestion + "</b>";
}
...
function actualizarTipoGestion(){
  ...
  fetch("http://144.126.210.74:8080/api/tipo_gestion/"+g_id_tipo_gestion, requestOptions)
    .then((response) => {
      if(response.status == 200){
        alert("Tipo de gestion actualizado correctamente");
        location.href="listar.html";
      }
      if(response.status == 400){
...
function eliminarTipoGestion(){
  ...
  fetch("http://144.126.210.74:8080/api/tipo_gestion/"+g_id_tipo_gestion, requestOptions)
    .then((response) => {
      if(response.status == 200){
        alert("Tipo de gestion eliminado correctamente");
        location.href="listar.html";
      }
      if(response.status == 400){
        alert("Error al eliminar tipo de gestion");
      }
    })
    .then((result) => console.log(result))
    .catch((error) => console.error(error));
